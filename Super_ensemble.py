...
from load_data import img_loader
...
# Data / Model
parser.add_argument('--data_path', metavar='DPATH', default='D:\pythonproject\pythonProject\\remote_class\data\swimcat',
                    type=str, help='Path to dataset')
parser.add_argument('--dataset', metavar='DSET', type=str,
                    choices=['cifar10', 'cifar100', 'swimcat', 'ImageNet10', 'GRSCD', 'MNIST300'], help='Choose between CIFAR-100/10.')
...
parser.add_argument('--size', type=int, default="224", help='piture_size')
...
# Optimization
parser.add_argument('--epochs', metavar='N', type=int, default=100, help='Number of epochs to train.')
...
def load_dataset():
    ...
    if args.dataset == 'cifar10':
        ...
        num_classes = 100
        train_loader, test_loader = None,None
    elif args.dataset == 'swimcat':
        # mean, std = [x / 255 for x in [129.3, 124.1, 112.4]], [x / 255 for x in [68.2, 65.4, 70.4]]

        mean, std = [0.28500387, 0.3225509, 0.35542664], [0.2935223, 0.3136111, 0.33660778]
        # train_loader, test_loader = load_data.load_imagefolder(args.data_path, mean, std, args)
        train_loader, test_loader = img_loader(args.data_path, mean, std)
        num_classes = 5
    elif args.dataset == 'ImageNet10':
        mean, std = [0.42849126, 0.44950873, 0.37706745], [0.2100645, 0.20778885, 0.19984259]
        # train_loader, test_loader = load_data.load_imagefolder(args.data_path, mean, std, args,size = args.size)
        train_loader, test_loader = img_loader(args.data_path, mean, std)
        num_classes = 10
    elif args.dataset == 'GRSCD':
        mean, std = [0.28500387, 0.3225509, 0.35542664], [0.2935223, 0.3136111, 0.33660778]
        # train_loader, test_loader = load_data.load_imagefolder(args.data_path, mean, std, args,size = args.size)
        train_loader, test_loader = img_loader(args.data_path, mean, std)
        num_classes = 7
    elif args.dataset == 'MNIST300':
        mean, std = [0.485, 0.456, 0.406], [0.229, 0.224, 0.225]
        train_loader, test_loader = load_data.load_imagefolder(args.data_path, mean, std, args,size=28)
        num_classes = 10
...
def load_model(num_classes, log, max_params, share_type, upsample_type,
        ...
        if args.dist:
            ...
            for student_net in student_nets:
                distributed_student_nets.append(torch.nn.DataParallel(
                    student_net.to(args.device)#, device_ids=list(range(args.ngpu))
                ))
    ...
    if args.dist:
        ...
        net = torch.nn.DataParallel(
            net.to(args.device)#, device_ids=list(range(args.ngpu))
        )
...
def learn_parameter_groups(train_loader, state, num_classes, log):
    ...
    if args.label_smoothing > 0.0:
        criterion = LabelSmoothingNLLLoss(args.label_smoothing).to(get_cuda_device())
    else:
    ...
    coefficients = torch.stack(kernel_coefficients['3_3']).to(args.device).numpy()
...
def main():
    ...
    summary(net, input_size=(3, args.size, args.size), batch_size=args.batch_size, verbose=1)
    count_model_param_flops(net)
    ...
    if args.evaluate:
        ...
        flops = gather_flops(input.to(get_cuda_device()), net, student_nets)
...
def analyze_gradients(train_loader, model, student_models, criterion, optimizer, state, reinit=False):
    ...
    for i, (input, target) in enumerate(train_loader):
        input = input.to(get_cuda_device())
        target = target.to(get_cuda_device())
...
def profile_models(train_loader, model, student_models, use_cuda = False):
    model.train()
    for student_model in student_models:
        student_model.train()
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        output = model(input)
        with profiler.profile(with_stack=True, profile_memory=True, record_shapes=True, use_cuda=use_cuda) as prof:
...
def save_checkpoint(state, is_best, save_path, filename):
    ...
    if is_best:
        bestname = os.path.join(save_path, '%s_model_best.spth.tar' % args.dataset)
