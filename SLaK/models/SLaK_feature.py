...
def get_conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias):
    ...
    return nn.Conv2d(in_channels, out_channels,kernel_size, stride, padding, dilation, groups,bias=bias)
...
class ReparamLargeKernelConv(nn.Module):
    ...
    def forward(self, inputs):
        if hasattr(self, 'lkb_reparam'):
            ...
            # print("self.LoRA1(inputs)", self.LoRA1(inputs).size())
            # print("self.LoRA2(inputs)22", self.LoRA2(inputs).permute(0, 1, 3, 2).size())
            out = self.LoRA1(inputs) + self.LoRA2(inputs)

            if hasattr(self, 'small_conv'):
                # print("self.small_conv(inputs)", self.small_conv(inputs).size())
